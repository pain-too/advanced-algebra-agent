"""
数据结构 RAG 核心服务
适配自原 RAG 项目的 RagService
改动点：
1. 配置从 YAML 读取（替代 config_data.py）
2. 模型从 factory 获取
3. 保留核心检索和格式化能力
"""
import os
...
from data.rag.vector_store import VectorStoreService


class DSRagService:
    """
    王道408数据结构 RAG 服务层
    提供PDF加载、向量检索、内容格式化、页码溯源
    """
    ...
    def pdf_upload_folder_with_md5(self, folder_path: str) -> None:
        ...
        try:
            ...
            logger.info(f"本次共加载 PDF 文件数量：{pdf_count}")
    ...
    def format_docs(self, docs: List[Document]) -> str:
        ...
        logger.info(f"开始格式化文档数量：{len(docs)}")
        formatted_list = []
